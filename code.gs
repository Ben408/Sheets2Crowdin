...
function getTranslationFromCrowdin(stringIdentifier, languageId, config) {
  ...
  if (languageId == "es-419"){
    ...
    language = "pt"  // Crowdin uses "pt" for Portuguese (Iberian)
  }
  else if (languageId == "pt-BR"){
    language = "pt-BR"  // Crowdin uses "pt-BR" for Brazilian Portuguese
  }
  else{
  ...
  try {
    ...
    Logger.log('Translation response: ' + (responseText ? responseText.slice(0, 500) : ''));    if (responseCode >= 200 && responseCode < 300) {
      const data = JSON.parse(responseText);
      if (data.data && data.data.length > 0) {
        Logger.log('Found ' + data.data.length + ' total translations');
        
        // Sort translations to prioritize approved ones, then by date
        const sortedTranslations = data.data.sort((a, b) => {
          const aApproved = a.data.approvalStatus === 'approved';
          const bApproved = b.data.approvalStatus === 'approved';
          
          // First priority: approved translations
          if (aApproved && !bApproved) return -1;
          if (!aApproved && bApproved) return 1;
          
          // Second priority: most recent (if dates are available)
        ...
        const bestTranslation = sortedTranslations[0];
        Logger.log('Selected translation: ' + bestTranslation.data.text + ' (Status: ' + bestTranslation.data.approvalStatus + ')');
        return bestTranslation.data.text;
